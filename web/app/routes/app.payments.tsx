...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return { 
    payments, 
    proofs: Array.isArray(proofs) ? proofs.filter((p: any) => p.status === "PENDIENTE") : [],
    page: Number(page) 
  };
};
...
export default function PaymentHistorial() {
  const { payments, proofs, page: loaderPage } = useLoaderData<typeof loader>() as { payments: PaymentListItem[], proofs: PaymentProof[], page: number };
  const submit = useSubmit();
  const navigation = useNavigation();
  
  const loading = navigation.state === "loading" || navigation.state === "submitting";
  const proofsLoading = false; // We get it immediately from loader
  ...
  const handleBatchReview = (status: string) => {
    ...
    submit({ intent: "batch-review", payment_ids: JSON.stringify(Array.from(selectedIds)), status }, { method: "post" });
  };

  const handleBatchDelete = () => {
    if (selectedIds.size === 0) return;
    if (!confirm(`¿Está seguro de eliminar ${selectedIds.size} pagos?`)) return;
    submit({ intent: "batch-delete", payment_ids: JSON.stringify(Array.from(selectedIds)) }, { method: "post" });
  };

  const handleBatchCancel = () => {
    if (selectedIds.size === 0) return;
    if (!confirm('¿Seguro que deseas cancelar los pagos seleccionados? Se revertirá en resuelto en el crédito asociado.')) return;
    submit({ intent: "batch-cancel", payment_ids: JSON.stringify(Array.from(selectedIds)) }, { method: "post" });
  };
  ...
  const handleApproveProof = (proof: PaymentProof) => {
    ...
    submit({ intent: "approve-proof", payment_id: proof.payment_id.toString(), proof_id: proof.id.toString() }, { method: "post" });
  };
  ...
  const handleRejectProof = (proof: PaymentProof) => {
    ...
    submit({ intent: "reject-proof", payment_id: proof.payment_id.toString(), proof_id: proof.id.toString() }, { method: "post" });
  };
  ...
  return (
    <s-page heading="Historial de Pagos" inlineSize="large">
      <s-button slot="primary-action" icon="plus" href="/app/registre_payment" accessibilityLabel="Ir a registrar pago">
        Registrar Pago
      </s-button>
      ...
      {proofs.length > 0 && (
        <s-section padding="base">
          <s-stack direction="inline" gap="base" justifyContent="space-between" alignItems="center">
            ...
            <s-button icon="delete" tone="critical" variant="secondary" onClick={handleClearProofs} accessibilityLabel="Vaciar todos los comprobantes">Vaciar Todo</s-button>
          </s-stack>
            ...
            <s-table-body>
              {proofs.map(p => (
                <s-table-row key={p.id}>
                  <s-table-cell><ClientDate dateString={p.submitted_at} format="datetime" /></s-table-cell>
                  <s-table-cell>
                      <s-stack gap="small">
                  ...
                  <s-table-cell>
                    <s-button-group>
                      <s-button icon="check" tone="auto" onClick={() => handleApproveProof(p)} accessibilityLabel="Aprobar comprobante de pago">Aprobar</s-button>
                      <s-button icon="delete" tone="critical" variant="secondary" onClick={() => handleRejectProof(p)} accessibilityLabel="Rechazar comprobante de pago">Rechazar</s-button>
                      <s-button icon="view" variant="secondary" href={`/app/payment_detail/${p.payment_id}`} accessibilityLabel="Ver detalles del pago">Ver Pago</s-button>
                      <s-button icon="view" variant="secondary" href={`/app/credit_detail/${p.credit_id}`} accessibilityLabel="Ver detalles del crédito">Ver Crédito</s-button>
                    </s-button-group>
                  </s-table-cell>
                </s-table-row>
              ))}
      ...
      <s-section padding="base">
        ...
        <s-table
          ...
          onNextPage={() => submit({ page: String(loaderPage + 1) }, { method: "get" })}
          onPreviousPage={() => submit({ page: String(Math.max(1, loaderPage - 1)) }, { method: "get" })}
        >
          <s-table-header-row>
              <s-table-header>
                <input
              ...
              <s-table-header format="numeric">Balance Restante Crédito</s-table-header>
            ...
            {payments.map((payment) => {
              ...
              const cuotasTarget = payment.installments_covered ? payment.installments_covered.split(',').filter((x: string) => x.trim()).length.toString() + " Cuota(s)" : "-";
              let cuotasCubiertas = "-";
              if (payment.installments_covered) {
                  cuotasCubiertas = payment.installments_covered.split(',').filter((x: string) => x.trim()).length.toString() + " Cuota(s)";
              }
